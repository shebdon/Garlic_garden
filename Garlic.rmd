...
#### Comparison to store-bougth cloves
...
This next block encodes the position of each clove.
``` {r}
...
``` {r, echo=FALSE}
p <- plot_ly(data = garlic, type = "scatter") %>%
  add_markers(x = ~from_east, y = ~from_north,
              marker = list(size = ~id+10),
              text = ~paste('Weight: ', id),
              hoverinfo = 'text',
              showlegend = F ) %>%
